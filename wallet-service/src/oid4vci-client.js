...
export class OID4VCIWalletClient {
     ...
     */
    async parseCredentialOffer(offerUri) {
        try {
     ...
     */
    async createProofOfPossession(audience, nonce, accessToken) {
        if (!this.walletDID) {
     ...
     */
    async getAccessToken(tokenEndpoint, preAuthorizedCode, userPin) {
        const tokenRequest = {
     ...
     */
    async requestCredential(credentialEndpoint, accessToken, credentialConfigurationId, nonce) {
        if (!this.walletDID) {
            throw new Error('Wallet not initialized');
        }
    ...
    async getStoredCredentials(type) {
        try {
            ...
            const result = credentials.map((cred) => ({
                ...
                hash: cred.hash,
                metadata: {
                    stored_at: cred.issuanceDate || new Date().toISOString(),
     ...
     */
    async receiveCredentialFromOffer(offerUri, userPin, issuerBaseUrl = 'http://localhost:8000') {
        ...
        try {
            ...
            const credentialResponse = await this.requestCredential(credentialEndpoint, tokenResponse.access_token, credentialConfigurationId, tokenResponse.c_nonce);
...
//# sourceMappingURL=oid4vci-client.js.map