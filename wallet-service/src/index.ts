...
app.post('/receive-credential', async (req, res) => {
  try {
    ...
    const result = await walletService.receiveCredential(
      offer_uri,
      user_pin,
...
app.get('/credentials', async (req, res) => {
  try {
    ...
    const credentials = await walletService.getStoredCredentials(type as string)
...
app.post('/present-credentials', async (req, res) => {
  try {
    ...
    const result = await walletService.respondToPresentationRequest(
      request_uri,
      auto_select,
      selected_credential_hashes
    )
...
app.post('/complete-ssi-flow', async (req, res) => {
  try {
    ...
    const result = await walletService.demonstrateCompleteSSIFlow(
