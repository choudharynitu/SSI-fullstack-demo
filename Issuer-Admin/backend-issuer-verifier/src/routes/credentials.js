...
function encodeJoseBlob(input) {
    return base64url.encode(JSON.stringify(input));
}
// Issue a credential with schema validation
router.post('/issue', async (req, res) => {
    const { issuerDID, subjectDID, credentialSubject, schemaId } = req.body;
    try {
        ...
        res.status(500).json({
            success: false,
            error: error.message || "An error occurred while issuing the credential",
        });
    }
});
...
router.post('/verify', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            success: false,
            error: error.message || "An error occurred while verifying the credential",
        });
    }
});
const getAlgFromVerificationMethod = (type) => {
    switch (type) {
...
router.post('/issuevcjwt', async (req, res) => {
    const { issuerDID, subjectDID, credentialSubject, schemaId } = req.body;
    try {
        // Load schema
        const schemas = loadSchemas();
        ...
        const signingKey = identifier.keys.find((k) => verificationMethod.id.endsWith(k.kid));
        if (!signingKey) {
