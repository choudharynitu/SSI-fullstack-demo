...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const where: any[] = []
    if (subjectDid) where.push({ column: 'subject', value: subjectDid })
    if (issuer) where.push({ column: 'issuer', value: issuer })
    if (type) where.push({ column: 'type', value: type })

    // Sort newest first if supported by your Veramo store
    const order: any = [{ column: 'issuanceDate', direction: 'DESC' }]

    const rows: any[] = await agent.dataStoreORMGetVerifiableCredentials({
