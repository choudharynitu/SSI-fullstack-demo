...
function haveCols(table) {
    return db.prepare(`PRAGMA table_info('${table}')`).all().map((r) => r.name);
}
function addCol(table, colDef, name) {
...
router.post('/offers', async (req, res) => {
    ...
    if (!schema)
        return res.status(400).json({ error: 'schema_not_found' });
    ...
    const cols = db.prepare(`PRAGMA table_info('offers')`).all().map((r) => r.name);
...
router.get('/oid4vci/credential-offer', (req, res) => {
    const offer_id = String(req.query.offer_id || '');
    if (!offer_id)
        return res.status(400).json({ error: 'offer_id required' });
...
router.post('/oid4vci/token', (req, res) => {
    const { grant_type, 'pre-authorized_code': preCode, user_pin } = req.body ?? {};
    if (grant_type !== 'urn:ietf:params:oauth:grant-type:pre-authorized_code') {
        return res.status(400).json({ error: 'unsupported_grant_type' });
    }
    if (!preCode)
        return res.status(400).json({ error: 'pre-authorized_code required' });
...
router.post('/oid4vci/credential', async (req, res) => {
    // 1) Bearer token
    ...
    if (!tok)
        return res.status(401).json({ error: 'invalid_token' });
    const now = Math.floor(Date.now() / 1000);
    if (tok.expires_at && now > tok.expires_at)
        return res.status(401).json({ error: 'token_expired' });
    const offer = db.prepare('SELECT * FROM offers WHERE id = ?').get(tok.offer_id);
    if (!offer)
        return res.status(400).json({ error: 'offer_not_found' });
    // 2) Request body
    ...
    if (!credential_subject?.id)
        return res.status(400).json({ error: 'credential_subject.id required' });
    if (!proof?.proof_type || !proof?.jwt)
        return res.status(400).json({ error: 'proof.jwt required' });
    ...
    const claims = parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed : {};
    const ok = validate(claims);
    if (!ok)
        return res.status(400).json({ error: 'claims_invalid', details: validate.errors });
    ...
    try {
        ...
        const toStore = (typeof verifiable === 'string') ? verifiable : verifiable;
